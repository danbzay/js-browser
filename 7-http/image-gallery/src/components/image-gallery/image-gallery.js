...
export class ImageGallery {
  constructor(element) {
    ...
    this.dropZone.addEventListener('drop', ev => {
      ev.preventDefault();
      this.addImagesToList([...ev.dataTransfer.items].map(
        item => item.getAsFile()).filter(file => file));
    });

    window.addEventListener('drop', ev => [...ev.dataTransfer.items].some(
      item => item.kind === 'file') ? ev.preventDefault() : 0);
  ...
  addImagesToList(files) {
    ...
    fetch(IMG_URL, {method: 'POST', body: formData}).then(res => res.json())
